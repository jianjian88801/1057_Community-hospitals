<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="检查类型" prop="jianchaleixing">
          <el-input v-model="ruleForm.jianchaleixing" 
              placeholder="检查类型" clearable  :readonly="ro.jianchaleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="检查类型" prop="jianchaleixing">
              <el-input v-model="ruleForm.jianchaleixing" 
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="检查项目" prop="jianchaxiangmu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="检查项目"
                  v-model="ruleForm.jianchaxiangmu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jianchaxiangmu" label="检查项目" prop="jianchaxiangmu">
                    <span>{{ruleForm.jianchaxiangmu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `daijianchaliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.daijianchaliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
