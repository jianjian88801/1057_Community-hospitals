  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.guanliyuanzhanghao!=''&&json.guanliyuanzhanghao){
              		this.ruleForm.guanliyuanzhanghao = json.guanliyuanzhanghao
		}
		if(json.guanliyuanxingming!=''&&json.guanliyuanxingming){
    ...
    info(id) {
      this.$http({
        url: `zhushequyaoliebiao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhushequyaoliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhushequyaoliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
