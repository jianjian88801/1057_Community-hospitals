<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="注射时间" prop="zhusheshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.zhusheshijian" 
                type="datetime"
                placeholder="注射时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.zhusheshijian" label="注射时间" prop="zhusheshijian">
              <el-input v-model="ruleForm.zhusheshijian" 
                placeholder="注射时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.zhusheshijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhusheyaowuliebiao = true;
          ...
          if(o=='zhusheshijian'){
            this.ruleForm.zhusheshijian = obj[o];
	    this.ro.zhusheshijian = true;
            continue;
          }
          if(o=='hushigonghao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.hushigonghao!=''&&json.hushigonghao){
              		this.ruleForm.hushigonghao = json.hushigonghao
		}
		if(json.hushixingming!=''&&json.hushixingming){
              		this.ruleForm.hushixingming = json.hushixingming
		}
        } else {
    ...
    info(id) {
      this.$http({
        url: `wanchengzhusheliebiao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wanchengzhusheliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wanchengzhusheliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
